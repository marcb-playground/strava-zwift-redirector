...
def move_watts_to_power(file_source_path, file_target_path):
    ...
    pattern = re.compile(r'(<extensions>)(.*?)(<gpxtpx:TrackPointExtension>.*?<gpxtpx:watts>)(\d+)(</gpxtpx:watts>)(.*?)(</gpxtpx:TrackPointExtension>.*?</extensions>)', re.DOTALL)
    ...
    def replace_watts_with_power(match):
        ...
        after_track_point_extension = match.group(6) + match.group(7)
        power_element = f'<power>{watts_value}</power>'
        return before_track_point_extension + power_element + after_track_point_extension
