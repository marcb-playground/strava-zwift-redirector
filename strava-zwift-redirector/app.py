...
STRAVA_ACTIVITY_NOTIFICATION_CALLBACK_URL = os.getenv('STRAVA_ACTIVITY_NOTIFICATION_CALLBACK_URL')  # e.g., "https://yourdomain.com/webhook"

source_client = get_strava_client(STRAVA_SOURCE_REFRESH_TOKEN)
...
def verify():
    ...
    return verify_webhook(hub_mode, hub_challenge, hub_verify_token, STRAVA_SOURCE_CLIENT_ID)

@app.route('/subscribe', methods=['POST'])
def subscribe():
    print(f"Subscribing for {STRAVA_SOURCE_CLIENT_ID}")
    subscription_url = 'https://www.strava.com/api/v3/push_subscriptions'
    return subscribe_to_webhook(subscription_url, STRAVA_SOURCE_REFRESH_TOKEN, STRAVA_ACTIVITY_NOTIFICATION_CALLBACK_URL)
